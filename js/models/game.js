...
start() {
  ...
  for (let round = 1; round <= numberOfRounds; round++) {
    ...
    // Display all words on the card to clue-givers (not the guesser)
    console.log("\nClue-givers, please look at the words on the card:");
    ...
    do {
      selectedWordIndex = parseInt(readline.question(`\n${guesser.name}, enter the number (1-5) of the word you want to guess: `));
    } while (isNaN(selectedWordIndex) || selectedWordIndex < 1 || selectedWordIndex > 5);
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      let proposal;
      do {
        proposal = readline.question(`${player.name}, enter your one-word proposal for word ${selectedWordIndex}: `).trim();
        if (proposal === '') {
      ...
      // Log the proposal
      const logEntry = `Round ${round} - ${player.name}: ${proposal} (Card ID: ${card.id}, Word Index: ${selectedWordIndex})\n`;
